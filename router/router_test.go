...
func TestRouter(t *testing.T) {
	...
	routed := false
	Handle("GET", "/user/:name", func(ctx *fasthttp.RequestCtx) {
		routed = true
...
func TestRouterAPI(t *testing.T) {
	...
	router := New()
	GET("/GET", func(ctx *fasthttp.RequestCtx) {
		get = true
	})
	HEAD("/GET", func(ctx *fasthttp.RequestCtx) {
		head = true
	})
	OPTIONS("/GET", func(ctx *fasthttp.RequestCtx) {
		options = true
	})
	POST("/POST", func(ctx *fasthttp.RequestCtx) {
		post = true
	})
	PUT("/PUT", func(ctx *fasthttp.RequestCtx) {
		put = true
	})
	PATCH("/PATCH", func(ctx *fasthttp.RequestCtx) {
		patch = true
	})
	DELETE("/DELETE", func(ctx *fasthttp.RequestCtx) {
		deleted = true
	})
...
func TestRouterChaining(t *testing.T) {
	...
	fooHit := false
	POST("/foo", func(ctx *fasthttp.RequestCtx) {
		fooHit = true
		ctx.SetStatusCode(fasthttp.StatusOK)
	})

	barHit := false
	POST("/bar", func(ctx *fasthttp.RequestCtx) {
		barHit = true
		ctx.SetStatusCode(fasthttp.StatusOK)
	})
...
func TestRouterOPTIONS(t *testing.T) {
	...
	// custom handler
	var custom bool
	OPTIONS("/path", func(_ *fasthttp.RequestCtx) {
		custom = true
	})
...
func TestRouterNotAllowed(t *testing.T) {
	...
	OPTIONS("/path", handlerFunc) // must be ignored
	...
	MethodNotAllowed = fasthttp.RequestHandler(func(ctx *fasthttp.RequestCtx) {
...
func TestRouterPanicHandler(t *testing.T) {
	...
	PanicHandler = func(ctx *fasthttp.RequestCtx, p interface{}) {
		panicHandled = true
	}
...
func TestRouterLookup(t *testing.T) {
	...
	handle, tsr := Lookup("GET", "/nope", ctx)
	if handle != nil {
	...
	GET("/user/:name", wantHandle)

	handle, tsr = Lookup("GET", "/user/gopher", ctx)
	if handle == nil {
...
func TestRouterServeFiles(t *testing.T) {
	...
	recv := catchPanic(func() {
		ServeFiles("/noFilepath", os.TempDir())
	})
	if recv == nil {
	...
	ServeFiles("/*filepath", os.TempDir())
