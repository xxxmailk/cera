...
func printChildren(n *node, prefix string) {
	fmt.Printf(" %02d:%02d %s%s[%d] %v %t %d \r\n", priority, maxParams, prefix, path, len(children), handle, wildChild, nType)
...
func checkRequests(t *testing.T, tree *node, requests testRequests) {
	for _, request := range requests {
		...
		handler, _ := getValue(request.path, requestCtx)
...
func checkPriorities(t *testing.T, n *node) uint32 {
	...
	for i := range children {
		prio += checkPriorities(t, children[i])
	}
...
func checkMaxParams(t *testing.T, n *node) uint8 {
	var maxParams uint8
	for i := range children {
		params := checkMaxParams(t, children[i])
		if params > maxParams {
			maxParams = params
		}
	}
	if nType > root && !wildChild {
		maxParams++
	}

	if maxParams != maxParams {
		t.Errorf(
			"maxParams mismatch for node '%s': is %d, should be %d",
			path, maxParams, maxParams,
		)
	}
...
func TestTreeAddAndGet(t *testing.T) {
	...
	for _, route := range routes {
		addRoute(route, fakeHandler(route))
	}
...
func TestTreeTrailingSlashRedirect(t *testing.T) {
	...
	for _, route := range tsrRoutes {
		handler, tsr := getValue(route, ctx)
		if handler != nil {
...
func TestTreeRootTrailingSlashRedirect(t *testing.T) {
	...
	recv := catchPanic(func() {
		addRoute("/:test", fakeHandler("/:test"))
	})
	if recv != nil {
		t.Fatalf("panic inserting test route: %v", recv)
	}

	handler, tsr := getValue("/", nil)
	if handler != nil {
...
func TestTreeFindCaseInsensitivePath(t *testing.T) {
	...
	for _, route := range routes {
		out, found := findCaseInsensitivePath(route, true)
		if !found {
	...
	for _, route := range routes {
		out, found := findCaseInsensitivePath(route, false)
		if !found {
			t.Errorf("Route '%s' not found!", route)
		} else if string(out) != route {
			t.Errorf("Wrong result for route '%s': %s", route, string(out))
	...
	for _, test := range tests {
		out, found := findCaseInsensitivePath(test.in, true)
	...
	// With fixTrailingSlash = false
	for _, test := range tests {
		out, found := findCaseInsensitivePath(test.in, false)
		if test.slash {
...
func TestTreeInvalidNodeType(t *testing.T) {
	...
	tree := &node{}
	addRoute("/", fakeHandler("/"))
	addRoute("/:page", fakeHandler("/:page"))
	...
	recv = catchPanic(func() {
		findCaseInsensitivePath("/test", true)
	})
	if rs, ok := recv.(string); !ok || rs != panicMsg {
		t.Fatalf("Expected panic '"+panicMsg+"', got '%v'", recv)
	}
